...
export default function Pagamento() {
  ...
  const submit = async (finalizar) => {
    ...
    // Validar se o valor do pagamento em dinheiro é maior que o saldo
    let temTroco = false;
    let valorTroco = 0;
  ...
  return (
    <div className="p-6 lg:p-8 fade-in max-w-4xl">
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-6 mb-6">
        ...
        {/* Aviso de troco */}
        {novos.some(p => p.forma_pagamento === "dinheiro" && Number(p.valor) > saldo && Number(p.valor) > 0) && (
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-start gap-2">
            <AlertCircle className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
            <div className="text-sm text-blue-700">
              <strong>Troco será calculado automaticamente</strong> quando você registrar o pagamento em dinheiro com valor superior ao saldo.
            </div>
          </div>
        )}
